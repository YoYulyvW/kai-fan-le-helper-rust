...
| 主题（浅色/深色/自动） | platform detect_system_theme + 托盘切换 | ⚠️ 检测已实现，配色渲染待完善 |
...
- Release 二进制体积：约 95–175 KB（目标 < 15 MB）✅
- 单元测试：16 个全部通过 ✅
- CI：GitHub Actions windows-latest 全绿 ✅
- 内存占用：待运行时实测（预计 < 10 MB，无 GC、无 webview）
...
1. 主题配色渲染（当前仅切换状态，控件着色未按主题绘制）。
2. 运行时缩放（常量已定义，控件未按 scale 重排）。
3. 映射弹窗的搜索过滤框。
4. 剪贴板实体化（延迟渲染固化）逻辑。
