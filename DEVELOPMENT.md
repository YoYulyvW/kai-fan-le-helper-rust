...
| 方案 | 外观 | 内存 | 体积 |
|------|------|------|------|
| 原生 Win32 控件 | Win98 灰控件 | ~14 MB | 1.4 MB |
| egui/eframe（当前） | 现代深色圆角 | ~187 MB | 4.5 MB |

当前采用 egui 以满足"现代化、炫酷"需求。若需压低内存，可切回原生方案。
...
## 实测数据（本地真机，2026-09-25）

最终方案：原生 Win32 自绘 UI。

- Release 二进制体积：约 1.4 MB
- 运行时内存：**16 MB**（低于 Python 版 33.7 MB）
- 单元测试：16 个全部通过
- 界面：深色圆角工具条、ClearType 中文、右上角置顶、无控制台

已真机验证：窗口显示、历史弹窗、剪贴板解析、扫描逻辑（日志确认扫 192.168.2.x）。
待验证：手机在线时的实际连接（当前手机 TCP 8848 关闭）。
